...
const guidelineKeywordMap = [
  ...
  {
    test: (text) => /adrenal incidentaloma|incidentaloma|инцидентал/i.test(text),
    preset: guidelineSourcePresets.eseAdrenalIncidentaloma,
  },
  {
    test: (text) => /aua.*stone|stone.*aua|kidney stones|surgical management of stones/i.test(text),
    preset: guidelineSourcePresets.auaStones,
  },
  {
    test: (text) => /asrm|male infertility|aua.*infertility|infertility.*aua/i.test(text),
    preset: guidelineSourcePresets.auaMaleInfertility,
  },
  {
    test: (text) =>
      /urolithiasis|stone|кам|гиперкальциур|гиперурикозур|гипероксалур|cystinuria/i.test(text),
    preset: guidelineSourcePresets.eauUrolithiasis,
  },
  {
    test: (text) =>
      /infection|uti|cystitis|prostatitis|epididym|orchitis|балан|кандид|герпес|трихомон|микоплазм/i.test(
        text
      ),
    preset: guidelineSourcePresets.eauInfections,
  },
  {
    test: (text) =>
      /sexual|reproductive|infertility|hypogonad|ejaculat|peyronie|priapism|androlog|fertility|varicocele|sperm|эрект|эякуляц|бесплод/i.test(
        text
      ),
    preset: guidelineSourcePresets.eauSexual,
  },
  {
    test: (text) =>
      /paediatric|pediatric|child|children|hypospadias|exstrophy|cryptorchid|enuresis|дет/i.test(
        text
      ),
    preset: guidelineSourcePresets.eauPaediatric,
  },
  {
    test: (text) => /trauma|fracture|rupture|травм/i.test(text),
    preset: guidelineSourcePresets.eauTrauma,
  },
  {
    test: (text) =>
      /luts|bph|incontinence|overactive|underactive|retention|дгпж|задерж/i.test(text),
    preset: guidelineSourcePresets.eauMaleLuts,
  },
  {
    test: (text) => /минздрав|россий|ркр|клинические рекомендации/i.test(text),
    preset: guidelineSourcePresets.ruPortal,
  },
  {
    test: (text) => /australia|usanz|urology\.org\.au/i.test(text),
    preset: guidelineSourcePresets.usanzPortal,
  },
];
...
const ultrasoundTemplates = {
  ...
  'urology:infections': {
    overview:
      'УЗ-оценка воспалительных изменений паренхимы и мочевых путей, исключение осложнений.',
    findings: [
  ...
  'urology:oncology': {
    overview:
      'Сонографическая стратификация объемного образования и признаков местного распространения.',
    findings: [
  ...
  'urology:nephrology': {
    overview:
      'УЗ-оценка размеров, кортико-медуллярной дифференцировки и сосудистых характеристик почек.',
    findings: [
  ...
  'andrology:sexual': {
    overview:
      'УЗ-оценка структур полового члена, простаты и органов мошонки по клиническому сценарию.',
    findings: [
  ...
  'pediatric:default': {
    overview:
      'Возраст-адаптированное УЗИ мочевых путей у детей с учетом врожденной аномалии и функциональных нарушений.',
    findings: [
...
export const buildLifestyleFallback = (disease) => {
  ...
  if (disease.section === 'urology' && disease.subsection === 'stones') {
    advice.push(
      'Поддерживать достаточную гидратацию в течение суток и ориентироваться на светлую мочу.',
      'Сохранять данные о перенесённых камнях и анализе состава, если он выполнялся.'
    );
    nutrition.push(
      'Ограничить избыток соли и ультраобработанных продуктов.',
      'Не ограничивать кальций самостоятельно без обсуждения с врачом.'
    );
    patient.push(
      'При температуре, ознобе, анурии или некупируемой боли обращаться срочно.',
      'Контроль рецидива строится не только на УЗИ, но и на образе жизни.'
    );
  }

  if (disease.section === 'urology' && disease.subsection === 'infections') {
    advice.push(
      'Не терпеть мочеиспускание длительное время и избегать переохлаждения.',
      'Соблюдать режим лечения до конца курса.'
    );
    nutrition.push(
      'Поддерживать адекватное потребление жидкости при отсутствии противопоказаний.',
      'Снизить алкоголь и раздражающие мочевой пузырь напитки при дизурии.'
    );
    patient.push(
      'Не начинать и не менять антибиотики самостоятельно.',
      'При лихорадке и боли в боку обращаться экстренно.'
    );
  }

  if (disease.section === 'urology' && disease.subsection === 'functional') {
    advice.push(
      'Вести дневник мочеиспусканий и отслеживать провоцирующие факторы.',
      'Регулярно оценивать симптомы и качество жизни на терапии.'
    );
    nutrition.push(
      'Ограничить поздний приём жидкости, кофеин и алкоголь при ноктурии и urgency.',
      'При запорах скорректировать пищевое волокно и водный режим.'
    );
    patient.push(
      'Не отменять терапию без оценки эффекта у врача.',
      'При нарастании остаточной мочи или задержке мочи обращаться раньше.'
    );
  }

  if (disease.section === 'urology' && disease.subsection === 'nephrology') {
    advice.push(
      'Контролировать давление, массу тела и функцию почек.',
      'Избегать нефротоксичных препаратов без необходимости.'
    );
    nutrition.push(
      'Ограничить избыток соли; при снижении функции почек корректировать рацион индивидуально.',
      'Не использовать белковые и БАД-нагрузки без обсуждения при ХБП.'
    );
  ...
  if (disease.section === 'urology' && disease.subsection === 'oncology') {
    advice.push(
      'Соблюдать график онкологического наблюдения без пропусков.',
      'Поддерживать физическую активность в переносимом объёме.'
    );
    nutrition.push(
      'Рацион должен поддерживать массу тела и белковый статус во время лечения.',
      'При системной терапии коррекция питания обсуждается индивидуально.'
    );
    patient.push(
      'Любое ухудшение самочувствия на фоне лечения обсуждать с командой сразу, не ждать контрольного визита.'
    );
  }

  if (disease.section === 'andrology' && disease.subsection === 'sexual') {
    advice.push(
      'Нормализовать сон, вес, физическую активность и сосудистые факторы риска.',
      'Снижать стресс и избегать самовольного приёма препаратов “для потенции”.'
    );
    nutrition.push(
      'Средиземноморский тип питания полезен для сосудистого и сексуального здоровья.',
      'Ограничить алкоголь и курение как факторы ухудшения сексуальной функции.'
    );
    patient.push(
      'Резкое изменение сексуальной функции требует оценки не только симптомов, но и кардиометаболических рисков.'
    );
  }

  if (disease.section === 'andrology' && disease.subsection === 'fertility') {
    advice.push(
      'Снизить перегрев мошонки, нормализовать сон и вес.',
      'Ограничить никотин и анаболические препараты.'
    );
    nutrition.push(
      'Поддерживать достаточное поступление белка, овощей и омега-3 в рамках общего здорового рациона.',
      'Не увлекаться строгими дефицитными диетами при планировании фертильности.'
    );
    patient.push(
      'Бесплодие — это вопрос пары, а не только одного пациента.',
      'Не откладывать обследование при отсутствии беременности более 12 месяцев.'
    );
  }

  if (disease.section === 'andrology' && disease.subsection === 'endocrine') {
    advice.push(
      'Повторять гормональные анализы в правильное время и при стабильном состоянии здоровья.',
      'Корректировать сон, массу тела и метаболические факторы до решения о гормональной терапии.'
    );
    nutrition.push(
      'Контроль массы тела и дефицитов питания помогает стабилизировать гормональный фон.',
      'Избегать самоназначения тестостерона и “гормональных бустеров”.'
    );
    patient.push(
      'Один анализ редко является основанием для окончательных решений.',
      'При эндокринных нарушениях важна системная, а не только симптоматическая коррекция.'
    );
  }
