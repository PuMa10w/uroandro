...
export default function useAppNavigationState({ addToHistory, setFavorites }) {
  ...
  const [navigationSource, setNavigationSource] = useState(() =>
    initialRoute.diseaseId ? 'direct_url' : null
  );
  ...
  const handleNavigate = useCallback(
    (section, subsection, diseaseId, options = {}) => {
      ...
      if (diseaseId) {
        const nextOrigin = options.origin ||
          (selectedDiseaseId
            ? diseaseOrigin
            : { section: activeSection, subsection: activeSubsection || null }) || {
            section,
        ...
        import('../data')
          .then(({ allDiseases: loadedDiseases, diseaseById }) => {
            const disease =
              ...
              loadedDiseases.find(
                (item) => item.section === section && item.subsection === nextSubsection
              );
  ...
  const closeDisease = useCallback(
    (options = {}) => {
