...
const hasValue = (form, field) =>
  form?.[field] !== '' && form?.[field] !== undefined && form?.[field] !== null;
...
export function buildSpermogramAssessment(form = {}) {
  ...
  if ((testosterone > 0 && testosterone < 10) || (fsh > 0 && fsh <= 3))
    abnormalities.push('endocrine');
  ...
  const primaryPattern = azo
    ...
    : abnormalities.filter((item) => ['oligo', 'astheno', 'terato'].includes(item)).length >= 2
      ? 'combined'
      : abnormalities.find((item) =>
          ['oligo', 'astheno', 'terato', 'lowVolume', 'leuko', 'highDfi', 'immuneFactor'].includes(
            item
          )
        ) || 'normal';

  const severity =
    primaryPattern === 'azo' || (concentration > 0 && concentration < 5)
      ? 'severe'
  ...
  const nodes = [
    makeNode({
      ...
      clinicalMeaning:
        'Один анализ не должен превращаться в диагноз без оценки сбора, воздержания, лихорадки, лекарств и повторяемости.',
      diagnostics: [
      ...
      treatment: [
        'Не начинать длительные эмпирические курсы до подтверждения клинического паттерна.',
      ],
    ...
    makeNode({
      ...
      condition:
        'Объем >= 1,4 мл, концентрация >= 16 млн/мл, общая подвижность >= 42%, морфология >= 4%.',
      ...
      treatment: [
        'Коррекция направляется ведущим паттерном, репродуктивным окном пары и обратимыми факторами.',
      ],
      nextStep: abnormalities.length
        ? 'Открыть активные ветки дерева ниже.'
        : 'Если беременность не наступает, обследовать пару параллельно.',
  ...
  if (primaryPattern === 'normal') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Нормальная спермограмма не исключает couple-factor, сексуальную дисфункцию, эндокринный фактор или женский фактор.',
        diagnostics: [
          'Параллельная оценка пары, овуляции, трубного фактора и времени бесплодия.',
          'Повторить анализ при изменении клиники или после лихорадки/токсического воздействия.',
        ],
        treatment: [
          'Оптимизация образа жизни, фертильного окна, коррекция сексуальной функции по показаниям.',
        ],
        nextStep:
          'Если возраст партнерши >=35 лет или бесплодие длительное, не затягивать fertility-консультацию.',
  ...
  if (abnormalities.includes('oligo') || primaryPattern === 'combined') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Чем ниже концентрация и чем больше сопутствующих отклонений, тем выше вероятность клинически значимого male factor.',
        diagnostics: [
          ...
          concentration < 5
            ? 'Кариотип; AZF-делеции особенно при признаках нарушенного сперматогенеза.'
            : 'Повторная спермограмма и гормональный профиль при стойком снижении.',
        ],
        treatment: [
          'Коррекция обратимых факторов; лечение клинического варикоцеле по показаниям; ART-стратегия с учетом TMC и пары.',
        ],
        nextStep:
          concentration < 5
            ? 'Ранний genetics + ART route.'
            : 'Оценить обратимые причины и динамику через один цикл сперматогенеза.',
        avoid:
          'Не назначать тестостерон мужчине с репродуктивным запросом без отдельного fertility-плана.',
        sources: SOURCE_BASE,
      })
    );
  }
  ...
  if (abnormalities.includes('astheno') || primaryPattern === 'combined') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Снижение подвижности часто связано с оксидативным стрессом, варикоцеле, воспалением, лихорадкой, токсинами или лабораторной преаналитикой.',
        diagnostics: [
          'Проверить жизнеспособность при полной неподвижности.',
          'Искать варикоцеле, инфекцию/воспаление, heat exposure, курение, ожирение.',
          'При тяжелой форме и респираторном анамнезе думать о нарушениях жгутика/цилиарной функции.',
        ],
        treatment: [
          'Лечение причины; снижение тепловой нагрузки; обсуждение ART при тяжелом или смешанном профиле.',
        ],
        nextStep: 'Сопоставить с концентрацией, морфологией и TMC.',
        avoid:
          'Не объяснять всю проблему одним “плохим движением” без оценки остальных параметров.',
        sources: SOURCE_BASE,
      })
    );
  }
  ...
  if (abnormalities.includes('terato') || primaryPattern === 'combined') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Изолированная морфология слабее предсказывает исход, но в сочетании с другими отклонениями усиливает male-factor.',
        diagnostics: [
          'Повторить морфологию в лаборатории с контролем качества.',
          'Исключить варикоцеле, токсические факторы, лихорадку и выраженный oxidative stress.',
          'При типичных моногенных фенотипах направить к репродуктологу/генетику.',
        ],
        treatment: [
          'Коррекция обратимых причин; при тяжелом смешанном факторе чаще обсуждается ICSI.',
        ],
        nextStep: 'Оценить, изолированная ли морфология или часть OAT-профиля.',
        avoid:
          'Не делать вывод о невозможности естественного зачатия только по одному показателю морфологии.',
        sources: SOURCE_BASE,
      })
    );
  }
  ...
  if (primaryPattern === 'combined') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Множественные отклонения имеют большую клиническую значимость, чем один изолированный параметр.',
        diagnostics: [
          'Полный мужской фактор: осмотр, гормоны, УЗИ мошонки по показаниям, genetics при тяжелой олигозооспермии.',
          'Оценить окно пары: возраст партнерши, длительность бесплодия, прежние ART-циклы.',
        ],
        treatment: [
          'Коррекция обратимых факторов параллельно с обсуждением ART, если TMC низкий или время ограничено.',
        ],
        nextStep: 'Собрать план на 8-12 недель и одновременно определить ART-порог.',
        avoid:
          'Не тратить месяцы на последовательные эмпирические добавки при тяжелом смешанном профиле.',
        sources: SOURCE_ALL,
      })
    );
  }
  ...
  if (primaryPattern === 'azo') {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Азооспермия требует подтверждения pellet-анализом и быстрого разделения обструкции, нарушения сперматогенеза и гипогонадотропного сценария.',
        diagnostics: [
        ...
        treatment: [
          'OA: реконструкция или sperm retrieval + ICSI; NOA: микро-TESE + ICSI по показаниям; HH: этиотропная гормональная индукция сперматогенеза.',
        ],
  ...
  if (abnormalities.includes('lowVolume')) {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Низкий объем может отражать неполный сбор, ретроградную эякуляцию, гипоандрогению или дистальную обструкцию.',
        diagnostics: [
          'Уточнить полноту сбора и воздержание.',
          'Постэякуляторная моча при подозрении на ретроградную эякуляцию.',
          'TRUS/УЗИ при кислой реакции, низкой фруктозе или подозрении на EDO.',
        ],
        treatment: [
          'Лечить причину: ретроградный маршрут, EDO, эндокринный фактор или техника сбора.',
        ],
        nextStep:
          ph > 0 && ph < 7.2 && form.fructose === 'low'
            ? 'Сильнее думать об EDO/семенных пузырьках.'
            : 'Сначала подтвердить преаналитику и ретроградный сценарий.',
  ...
  if (abnormalities.includes('leuko')) {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Round cells нужно отличать от germ cells; антибиотик нужен не цифре, а подтвержденной инфекции.',
        diagnostics: [
          'Подтвердить лейкоциты пероксидазным тестом или другим валидным методом.',
          'Посев/ПЦР и оценка простатита/эпидидимита по клинике.',
          'Исключить стерильное воспаление и контаминацию.',
        ],
        treatment: [
          'Терапия по подтвержденному возбудителю и клинике; контроль спермограммы после лечения.',
        ],
  ...
  if (abnormalities.includes('highDfi')) {
    nodes.push(
      makeNode({
        ...
        condition: `DFI: ${dfi}%.`,
        clinicalMeaning:
          'DFI может быть полезен при повторных потерях, неудачах ART и выраженном oxidative-stress контексте, но не должен быть рутинным первым тестом.',
        diagnostics: [
          'Искать варикоцеле, курение, ожирение, лихорадку, воспаление, токсическое и тепловое воздействие.',
          'При повторных ART failure/RPL обсуждать расширенную male-factor оценку.',
        ],
        treatment: [
          'Коррекция обратимых факторов; лечение клинического варикоцеле по показаниям; ART-стратегия индивидуально.',
        ],
        nextStep:
          artFailures >= 2
            ? 'Связать DFI с ART-историей и эмбриологическим отчетом.'
            : 'Использовать как дополнительный, а не единственный драйвер решения.',
  ...
  if (abnormalities.includes('immuneFactor')) {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Высокий MAR может снижать естественное оплодотворение и усиливать аргументы в пользу ICSI при клинически значимом бесплодии пары.',
        diagnostics: [
          'Проверить контекст: травма, воспаление, операции, варикоцеле, обструкция.',
          'Повторить/подтвердить при несоответствии клинике.',
        ],
        treatment: [
          'Обсуждать ICSI route при стойком высоком MAR и неэффективности менее инвазивной стратегии.',
        ],
        nextStep: 'Сопоставить MAR с TMC, длительностью бесплодия и возрастом партнерши.',
        avoid:
          'Не строить длительное лечение только вокруг антител без репродуктивной стратегии пары.',
  ...
  if (form.varicocele === 'yes') {
    nodes.push(
      makeNode({
        ...
        status: abnormalities.some((item) =>
          ['oligo', 'astheno', 'terato', 'highDfi'].includes(item)
        )
          ? 'attention'
          : 'neutral',
        condition: 'Клиническое варикоцеле указано в форме.',
        clinicalMeaning:
          'Польза коррекции выше при клиническом варикоцеле, бесплодии пары и отклонениях спермограммы.',
        diagnostics: [
          'Подтвердить клиническую значимость, сторону, степень, объем яичек и боль.',
          'Оценить женский фактор и репродуктивное окно.',
        ],
        treatment: [
          'Микрохирургическая коррекция по показаниям; контроль спермограммы через 3-6 месяцев.',
        ],
        nextStep:
          'Решать вместе с возрастом партнерши, длительностью бесплодия и тяжестью male-factor.',
  ...
  if (abnormalities.includes('endocrine')) {
    nodes.push(
      makeNode({
        ...
        clinicalMeaning:
          'Низкий тестостерон или низко-нормальный ФСГ могут указывать на гипогонадотропный сценарий, особенно при азооспермии.',
        diagnostics: [
          'Повторить утренний тестостерон, ЛГ, ФСГ, пролактин, ТТГ по показаниям.',
          'Исключить анаболики, опиоиды, гиперпролактинемию, ожирение, системные заболевания.',
        ],
        treatment: [
          'При репродуктивной цели избегать экзогенного тестостерона; рассмотреть стимуляцию оси у профильного специалиста.',
        ],
        nextStep: 'Отделить функциональный дефицит от органического гипогонадизма.',
        avoid:
          'Не назначать TRT мужчине, который планирует зачатие, без сохранения/стимуляции фертильности.',
        sources: ['EAU', 'AUA_ASRM', 'RU'],
      })
    );
  }
  ...
  nodes.push(
    makeNode({
      ...
      status:
        severity === 'severe' || partnerAge >= 35 || artFailures >= 2 ? 'critical' : 'neutral',
      condition: `TMC: ${tmc || 'не указан'} млн; возраст партнерши: ${partnerAge || 'не указан'}; ART failure: ${artFailures || 0}.`,
      clinicalMeaning:
        'Маршрут IUI/IVF/ICSI зависит от TMC, тяжести male-factor, возраста партнерши, длительности бесплодия и предыдущих циклов.',
      diagnostics: [
        'Собрать female/couple factor параллельно.',
        'При повторных ART failures оценить эмбриологический отчет, RPL/RIF-контекст и male-factor add-ons с осторожностью.',
      ],
      treatment: [
        tmc >= 10 && severity === 'mild'
          ? 'IUI можно обсуждать при благоприятном female factor.'
          : 'IVF/ICSI вероятнее при низком TMC, тяжелом или смешанном male-factor.',
        primaryPattern === 'azo'
          ? 'При азооспермии обсуждать sperm retrieval + ICSI или donor route по типу азооспермии.'
          : 'Коррекцию обратимых факторов вести параллельно с репродуктивным планом.',
      ],
      nextStep:
        partnerAge >= 35 || artFailures >= 2
          ? 'Сократить окно наблюдения и быстрее собрать multidisciplinary fertility route.'
          : 'Переоценка через 8-12 недель после коррекции обратимых факторов.',
      avoid: 'Не противопоставлять лечение мужчины и ART, если время пары ограничено.',
      sources: SOURCE_ALL,
    })
  );

  const redFlags = nodes.filter((node) => node.status === 'critical').map((node) => node.title);
  ...
  return {
    ...
    patientSummary: [
      summary,
      severity === 'severe'
        ? 'Не затягивать углубленное обследование и обсуждение репродуктивного маршрута.'
        : 'Переоценивать динамику после коррекции обратимых факторов.',
      partnerAge >= 35
        ? 'Возраст партнерши делает время самостоятельным клиническим фактором.'
        : 'Срок наблюдения можно держать в рамках одного цикла сперматогенеза, если нет срочных факторов.',
    ],
    summary,
    nodes,
    sources: SOURCE_PACK,
