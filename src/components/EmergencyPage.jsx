...
const emergencyConditions = [
  {
    ...
    steps: [
      {
        step: 1,
        text: 'Клинически подозревать перекрут при внезапной боли и высоком положении яичка.',
        action: true,
      },
      ...
      {
        step: 5,
        text: 'Орхипексия с обеих сторон обязательна при сохраненном органе.',
        action: true,
      },
  ...
  },
  {
    ...
    steps: [
      {
        step: 1,
        text: 'Разделить ишемический и неишемический вариант: анамнез, осмотр, газовый состав кавернозной крови.',
        action: true,
      },
      { step: 2, text: 'При ишемическом приапизме не откладывать декомпрессию.' },
      {
        step: 3,
        text: 'Аспирация крови из кавернозных тел и промывание физиологическим раствором.',
        action: true,
      },
  ...
  },
  {
    ...
    steps: [
      {
        step: 1,
        text: 'Диагноз клинический: не задерживать операцию ожиданием визуализации.',
        action: true,
      },
      {
        step: 2,
        text: 'Немедленно начать широкую антибактериальную терапию и интенсивную поддержку.',
        action: true,
      },
      ...
      {
        step: 5,
        text: 'Контролировать источник инфекции, сахар крови, гемодинамику и признаки сепсиса.',
      },
  ...
  },
  {
    ...
    steps: [
      {
        step: 1,
        text: 'Подтвердить переполнение мочевого пузыря и выраженный дискомфорт.',
        action: true,
      },
      {
        step: 2,
        text: 'Немедленная катетеризация - первый шаг в большинстве случаев.',
        action: true,
      },
      {
        step: 3,
        text: 'Если трансуретральный катетер не проходит, не усиливать травму повторными попытками.',
      },
      {
        step: 4,
        text: 'При сложной катетеризации - уролог, проводник или цистостомия по показаниям.',
        action: true,
      },
      {
        step: 5,
        text: 'После декомпрессии искать причину: ДГПЖ, стриктура, неврология, лекарства, инфекция.',
      },
      {
        step: 6,
        text: 'Определить дальнейший маршрут: trial without catheter, альфа-блокатор, операция или дообследование.',
      },
    ],
  ...
  const heroActions = [
    {
      ...
      onClick: () =>
        onNavigate('urology', 'reconstructive', 'testicular-torsion', {
    ...
    },
    {
      ...
      onClick: () =>
        onNavigate('andrology', 'sexual', 'priapism', { source: 'service_emergency_hero' }),
    },
    {
      label: 'Острая задержка мочи',
      meta: 'Functional obstruction route',
      onClick: () =>
        onNavigate('urology', 'functional', 'urinary-retention', {
          source: 'service_emergency_hero',
        }),
    },
  ];
      ...
      <div className="emergency-grid">
        {emergencyConditions.map((condition) => (
          <div
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && setSelected(selected === condition.id ? null : condition.id)
            }
            role="button"
            tabIndex="0"
            ...
            {selected === condition.id ? (
              <div className="emergency-steps">
                {condition.steps.map((step) => (
                  <div
                    key={step.step}
                    className={`emergency-step ${step.action ? 'step-action' : ''}`}
                  >
              ...
              <p className="emergency-hint">
                Нажмите, чтобы раскрыть алгоритм и увидеть следующий маршрут
              </p>
            )}
          </div>
        ))}
